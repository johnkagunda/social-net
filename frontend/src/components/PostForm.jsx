...
export default function PostForm({ onPostCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (privacy === "private") {
      selectedViewers.forEach((id) => {
        formData.append("allowed_viewers", id);
      });
    }
  ...
  const handleViewerToggle = (id) => {
    if (selectedViewers.includes(id)) {
      setSelectedViewers(selectedViewers.filter((v) => v !== id));
    } else {
        ...
        <div style={{ flex: 1 }}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "12px", alignItems: "center" }}>
              ...
              <button type="submit" className="btn-primary" style={{ padding: "6px 24px" }}>
                Post
              </button>
            </div>
